...
fn disp<'a>(graph: &'a Graph, id: &'a str) -> &'a str {
    ...
    match graph.get(id) {
        ...
        Some(Node::Place(p)) => p.name_ru.as_deref().unwrap_or(&p.name),
        Some(Node::Enemy(e)) => e.name_ru.as_deref().unwrap_or(&e.name),
        Some(Node::Location(l)) => l.name_ru.as_deref().unwrap_or(&l.name),
